...
export default function Page() {
	...
	} = useApi(async (username: string, otp: string) => await AuthEndpoints.checkForgetPasswordOTP(username, otp));
	...
	useEffect(() => {
		if (checkOTPResult) {
			if (checkOTPResult?.success) {
				router.push(`/auth/change-password/set-password?username=${formValues?.username}&backUrl=${searchParams?.backUrl ?? ""}`);
			} else {
				showNotification({
					type: "error",
			...
			<form className="max-lg:!p-4 h-full flex justify-center flex-col max-lg:!-mt-16" onSubmit={checkOTPHandler}>
				<div className="max-lg:!min-h-[calc(100dvh-76px)]">
					...
					<div className="mt-1 text-center text-sm">
						کد تایید برای شماره {searchParams?.username} ارسال شد، برای تغییر رمز عبور آن را وارد کنید
					</div>
					<div className="mt-4">
						<TabanInput
							isLtr
